...
class ColumnistTemplate extends ITemplate {
     ...
     */
    createFile(path, colunist) {
        // verificacao param param obrigatorio: template
        if (colunist == null || typeof colunist !== 'object')
            throw new Error('[Param:data] paramentro obrigatÓrio do tipo [ARRAY]!')
    
        try {
            const content = this.template(colunist)
            const pathFolder = NewsletterFile.createFolder(path, colunist.slug)
     ...
     */
     createFiles(data) {
        ...
        data.forEach((colunist) =>  this.createFile('./ARQUIVOS-GERADOS/', colunist));
    }
}

module.exports = ColumnistTemplate